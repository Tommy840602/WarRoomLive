...
| `agenda` | The shared to-do list and calendar between two participants: an item added by one appears for the other **without a reload** (it travels over signaling), both see the same server-owned ordering, completing sinks an item below what is still open, and a calendar entry lands under the right day. |
