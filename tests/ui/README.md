...
| `agenda` | The shared to-do list and calendar between two participants: a whole item captured from **one line** (`訂會議室 @Bob 3天後`) puts the right assignee and due date on the wire, it appears for the other participant **without a reload** (it travels over signaling), both see the same server-owned ordering, completing sinks an item below what is still open, and a calendar entry lands under the right day. |
| `layout` | The workspace layout and the skin it wears: exactly one sidebar panel is open at any width, the tabs match the panels that actually exist, nothing spills off the side, video tiles stay legible on a phone — and the day/night skins really are different surfaces, the video tile stays black in both, and an explicit skin choice survives a reload. |
