...
| `agenda` | any stack | The shared to-do list and calendar: server-owned ordering (open first, soonest-due first, undated last), completion recorded as a time and an author that a repeat click cannot rewrite, partial updates that leave untouched fields alone, refusal of empty text / unparseable dates / an entry that ends before it starts, the calendar reading forwards from now, and the room being told over signaling which list changed. |
| `retention` | any stack | Expired data is deleted and everything else is left alone. The interesting assertions are the negative ones — recent rows survive, and an *unpublished* outbox row survives however old it is, because that is the queue rather than history. Covers every swept table: recordings and files with their objects, chat with its search projection, the to-do list and calendar, the audit log and published outbox rows. Runs a second backend via `docker compose run` with retention turned on, so the stack under test is never reconfigured. |
