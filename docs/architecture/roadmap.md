...
| 平面 | 藍圖建議 | 目前實作 | 差距 |
|---|---|---|---|
| 媒體 | LiveKit SFU + coturn | 模式切換:預設 mesh(≤8),`sfu` 功能走 LiveKit(上限 50);simulcast + adaptiveStream + dynacast;coturn TURN fallback;Egress 錄影 → MinIO + **清單與預簽 URL 播放**;**每對等連線品質量測與弱網降級、ICE 重啟** | TURN/TLS 443 需真實憑證(部署層) |
...
| 持久化 | PostgreSQL 為 source of truth | 聊天、筆記快照、outbox、稽核與搜尋讀模型、會議領域皆入 Postgres;Flyway 管 schema(V1–V13);WAL 歸檔 + PITR + 加密物件儲存歸檔 | 跨區域複寫、KMS(部署層) |
...
2. **認證授權** ✅ 大致完成:OIDC Resource Server(`oidc` profile)、collab JWT 驗證、前端 PKCE 登入 + **silent renew**(refresh token 單次使用輪替)、WS **逐訊息 token 過期強制**(4401 切斷)、`oidc` 功能(本機用 devidp;`prod oidc` 強制要求真實 Keycloak/Entra `OIDC_*` 且不啟動 devidp)。**資源層級授權(房間層)✅**:開房者為主持人(離開自動交接,backplane 原子維護 host/locked meta),LiveKit token 綁定已加入的 peer + JWT subject + 伺服器驗證名稱；錄影、檔案、逐字稿、摘要、會議、待辦與行事曆都要求 live membership，刪除與錄影控制再要求主持人，空房／缺 subject fail-closed；錄影停止 capability 綁定 room/Egress/actor。登入模式暫停全域搜尋，避免跨房間洩漏。尚餘(刻意遞延):持久 room membership/歷史存取 ACL、部門/專案層級 ACL(需組織目錄整合)、真實 IdP 對接演練。
3. **SFU 遷移** ✅ 大致完成:`sfu` 功能導入 LiveKit,前端 `SfuRoom`(與 mesh 同介面,由後端 `/api/media/config` 決定模式)、後端房間限定 token 簽發、信令上限放寬到 50、**simulcast + adaptiveStream + dynacast**;mesh 路徑的 coturn/TURN fallback(`turn`)。**錄影 ✅**(`recording`:LiveKit Egress room-composite → MinIO,webhook 驗簽 → `meeting.recording.completed` 事件)。尚餘(刻意遞延):TURN/TLS 443(需真實憑證)。
4. **水平擴展 backplane** ✅ 部分完成:`Backplane` 抽象(預設單機 no-op),`redis` profile 將成員目錄移入 Redis(節點心跳 + 惰性 ghost 清理)、跨節點訊息走 Pub/Sub、房間上限原子化(本地 compute / Redis Lua);collab 以 `extension-redis` 多實例同步;`scale` 跑 backend×2 + collab×2;**Kafka + transactional outbox**(`kafka` profile + Redpanda,at-least-once + eventId 去重;事件:chat.message.created、participant.joined/left、document.snapshot.created)+ **消費端範例 `indexer/`**(audit_log 稽核軌跡 + message_search 全文檢索讀模型,`GET /api/search/messages`)。另有**事件信封契約**(`docs/contracts/warroom-event.schema.json`,indexer 以 ajv 強制,違約 = 毒訊息)。**會議領域與事件 ✅**(`meetings` 表 + `meeting.started`/`meeting.ended`/`participant.rejected`,backplane 回傳叢集人數保證第一人/最後一人判定)。**Redis Sentinel 高可用 ✅**(`ha` 疊在 `scale` 之上:master + replica + 3 Sentinel quorum 2;後端 `redisha` profile 走 `spring.data.redis.sentinel.*`、collab ioredis 走 Sentinel;`tests/ha/failover-drill.sh` 實測殺 master 後自動升級,信令/CRDT 不中斷)。**Schema Registry ✅**(Redpanda 內建 registry,subject `warroom.events-value`;indexer 啟動註冊 + 以 registry latest 驗證,registry 不可達時退回內帶副本)。尚餘(刻意遞延):Redis Cluster(資料分片規模需求出現時)、OpenSearch 取代 Postgres FTS(搜尋規模需求出現時)。
5. **可觀測性** ✅ 大致完成:backend/collab/indexer Prometheus 指標、**OTLP 分散式追蹤 → Tempo**(`observability` 啟用,Grafana datasource 已接)、**LiveKit WebRTC 品質指標**接入 scrape、**Grafana dashboard** 與**告警規則**(target down / backlog / 拒連 / 處理延遲 SLO)。**Alertmanager 通知路由 ✅**(分組、critical 快速通道、target-down 抑制同 job warning;dev 接收端為 webhook logger,實測告警送達)+ **dashboard 補齊**(SFU 房間/參與者與 NACK/封包率、outbox backlog 與發布率、Redis backplane 指令延遲 p99、indexer consumed/failed/deduped)。尚餘:正式環境接 Slack/PagerDuty 憑證(部署層)。
...
8. **備份與 DR** ✅(藍圖 §十一,本機形態):`backup`(WAL 歸檔)+ `tests/dr/` 一鍵 PITR 還原演練(災前/災後標記、CRDT snapshot⊕log 重建 hash 比對;首次演練即抓到歸檔權限缺陷並修復)。詳見 `docs/runbooks/disaster-recovery.md`。**物件儲存歸檔 + 備份加密 ✅**(`backup-s3`:rclone shipper 經 `crypt` remote 客戶端加密同步進 MinIO,`tests/dr/restore-s3-drill.sh` 只從 bucket 還原並斷言 bucket 內無明文路徑;演練抓到「物件儲存丟失空目錄導致還原叢集拒啟動」,改用 tar 格式基礎備份修復)。尚餘:跨區域複寫、金鑰管理服務接入(部署層)。
...
**O. 兩個上線後回報的問題** ✅ 已完成:一個是隱形按鈕,一個是設計錯誤。

- **看不到文字**:`.app__controls button` 把每顆按鈕都塗上 `--accent-ink` —— 那是**放在主色上**的墨色(夜近黑、日近白)。`.btn-secondary` 只換了背景沒換墨色,結果夜間是深字配深底(1.15:1)、日間是**白字配白底**(1.04:1):emoji 照常渲染,字整個消失。所有既有測試都過——按鈕存在、可點、label 正確、完全看不見。修掉之外,`tests/ui/layout` 加了一個**量實際畫出來的對比度**的檢查(WCAG 公式、AA 門檻 4.5:1、兩個 skin 都量),而且先在未修的 CSS 上驗證它會抓到:1.15/1.04 —— 跟離線算的一模一樣。
...
**Q. 一個啟動檔案,所有功能** ✅ 已完成:12 個 overlay 檔案合併成一份 `docker-compose.yml`,功能改用開關,不再用疊的。

- **選用的「服務」用 compose `profiles:`;選用的「設定」不行** —— compose 沒有針對單一 key 的條件式,所以它們一律以「關閉」的預設值寫在檔案裡,由 `stack.sh` 提供打開時的值。這是單一檔案真正的代價,也是 launcher 還在的理由:功能 → profile + 設定 + Spring profile 的對照表,在一個畫面上。
- **合併前先實測四個機制**,猜錯的話整份檔案都要重寫:`depends_on` 指向未啟用 profile 的服務會讓**整個 project** 失效(硬關卡,所以 frontend 不再依賴 devidp/livekit、backend 不再依賴 redpanda/redis);`deploy.replicas` 和 port 的 bind address 可以用變數;同一個服務可以掛多個 profile(所以一個 redis 同時服務 scale/ha/recording)。
- **port 沒辦法用變數關掉** —— 空值會變成隨機 host port,不是「不發布」。所以 `tls`/`prod` 改成綁 loopback,而不是複製出第二份 frontend 服務定義。
- **同一個陷阱換了個形狀又咬了一次。** 上一輪的教訓是「測試 shell 出去的 compose 指令必須描述正在跑的那個 stack」,當時缺的是**檔案清單**。改成單一檔案之後,缺的變成**環境變數** —— `.stack.env` 有記 profile 但沒被載入,於是 retention 的 sweeper 又一次把 collab 重建成 events 關閉的版本,`document.snapshot.created` 停止產生,什麼都沒 log。**檔案清單本身會自我描述,一組環境變數不會**,這是舊模型唯一真正的優點。`lib.mjs` 現在在 import 時載入 `.stack.env`,而 `tests/compose/run.sh` 用 fixture 直接測這個載入行為 —— 這個陷阱咬過兩次了。
- 中途我先假設那是 debounce 的 race 並把等待從 4s 拉到 6s;三次重跑都還是失敗,證明假設錯了,所以那個改動**收回**了 —— 真正的原因在環境變數,4s 一直都夠。
- 驗證:`tests/compose/run.sh` 54 項;單一檔案起 base+ai+events+observability 時 12 支 e2e 全過(125 項);另外起 base+oidc+ai+events+observability 確認後端同時掛著四個 profile,無 token 401、有 token 200。
