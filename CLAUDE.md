...
- `docker-compose.events.yml` — opt-in event backbone: backend profiles `postgres,kafka`, Redpanda broker. Business events commit transactionally into `outbox_events` (`events/` package, `OutboxRecorder` inside the aggregate's transaction) and `OutboxPublisher` ships them at-least-once to topic `warroom.events` — consumers dedupe on `eventId`. The `kafka` profile requires `postgres`. To emit a new event type: record it inside the owning transaction, bump `schemaVersion` on shape changes.
