...
- **Room membership is backplane-abstracted**: `RoomManager` always tracks this node's local sessions; the `Backplane` interface answers cluster-wide questions. Default = `LocalBackplane` (single process, publishing no-ops). The `redis` profile swaps in `RedisBackplane`: membership directory in Redis hashes (entries carry the hosting node id), cross-node relay/broadcast over pub/sub channel `warroom:signal`, node heartbeats with lazy ghost pruning. When touching `SignalingHandler`, keep the pattern: deliver to local sessions via `RoomManager`, then `backplane.publish*` for other nodes — never double-deliver. The room-cap check is non-atomic cluster-wide (documented in `RedisBackplane`).
