...
- **一行輸入**:待辦與行事曆各只有一個輸入框,整句寫進去就好——`寄簡報 @bob 明天15:00`、`對外說明稿定稿 週三14:00`、`回覆法務 3天後`。負責人、日期、時間由前端從那一行解析出來(今天/明天/後天、週X/禮拜X/星期X、`HH:MM` 與 `HH點`、`M/D`、`N天後`/`N小時後`);**認不出來的字一律留在文字裡**,不會被吃掉。送出前把「解讀為」顯示回去,所以解析永遠不是事後才發現的猜測。會議進行中沒有人會 tab 過四個欄位,而那正是這份清單該被填的時刻。每一列以「還有多久」開頭、固定同一欄,所以清單是往下讀的而不是每行橫著讀;絕對時間放在 tooltip。行事曆沒有時間就放不上去,所以它會直說「還需要時間」,而不是留一顆沒有反應的按鈕。
