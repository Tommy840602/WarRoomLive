...
- **草稿丟掉,定稿留下**。辨識過程中每秒會改好幾次,那些只轉發、不落地也不翻譯——跟 CRDT 平面上的游標一樣。只有**定稿**寫進 `transcript_lines`,拿到 id,再連 id 一起廣播回房間(包含說話的人自己:他的字幕是自己畫的,沒有那個 id 就對不上稍後回來的翻譯)。
- **字幕不會等翻譯**。翻譯是另一則 `caption-translated` 訊息,晚一點到、用 id 對回那一行。一個要等語言模型回話才出現的字幕,已經不叫字幕了。翻譯排在一個小的有界佇列上,塞滿就**丟掉而不是排隊**——遲到兩分鐘的翻譯不是慢,是在幫對話的另一段配字幕。
- **讀者的語言在上面**。選了 English 的人是在讀英文、瞄一眼中文;把原文放上面只因為它是原文,等於每一行都逼他先跳過一行看不懂的。另一種語言仍然留著、但比較安靜——翻譯偶爾是錯的,看得到原文的房間才抓得到。
